...
int main(int argc, char **argv)
{
  ...
  size_t max_size = output_proto == UDP ? sizeof (struct in_packet): 1024 * 1024;

  struct in_packet* inpkt = (struct in_packet*) buffer;
  short *inpayload = (short*)(((struct in_packet*)buffer)->payload);
  struct out_packet outpkt;
  char *outpayload = ((char*)(&outpkt.payload));
  ...
  { if (read_size < 0) 
    ...
    { memcpy (&outpkt.header, &inpkt->header, sizeof (struct header));
      int sb = 2;
      // 12 antennas, 5 subbands each
      // NOTE: The 61st subband is not assigned, so that every packet has 5
      // subbands from 12 antennas.
      for (int dip=0,ant=0; dip<60; dip+=5,ant++) 
      { for (unsigned i = 0; i < 1 * 16 * 2 * 2; i ++) 
        {
          short value = inpkt->payload[dip+sb][0][0][i]; // Subband 1
          ...
          outpkt.payload[2*ant][0][0][i] = value & 255;

          value = inpkt->payload[dip+sb+1][0][0][i]; // Subband 2
          ...
          outpkt.payload[2*ant+1][0][0][i] = value & 255;
        }
      }
      write_size = 0;
