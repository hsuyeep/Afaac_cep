  ...
  while ((read_size = read(sk_in, buffer, max_size)) != 0) {
    ...
    { char *outpayload = ((char*)(&outpkt.payload));
  	...
  	value = 127, ++ bad;
        ...
        outpayload[i] = value & 255;
      }
      write_size = 0;
      while (write_size < sizeof (outpkt)) {
        if ((write_size=write(dipole_fd[dip], &outpkt, sizeof (outpkt))) < 0) 
	{ perror("write");
  	  sleep(1);
  	} 
        else 
