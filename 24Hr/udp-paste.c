...
void init(int argc, char **argv)
{
  ...
  // sk_in  = create_fd(argv[arg], 1, &input_proto, source, sizeof source);
  // sk_out = create_fd(argv[arg + 1], 1, &output_proto, destination, sizeof destination);

  // sk_in  = create_fd(argv[arg+1], 0, &input_proto, source, sizeof source);
  sk_out = create_fd(argv[arg], 1, &output_proto, destination, sizeof destination);
...
int main(int argc, char **argv)
{
  ...
  struct in_packet *inpkt = NULL;
  init(argc, argv); // input file handling is pulled out.
  fraw = fopen (argv[2], "rb");
...
/*
  while ((read_size = read(sk_in, buffer, max_size)) != 0) {
    if (read_size < 0) {
      ...
      sleep(1);
*/
  inpkt = (struct in_packet*)buffer;
  while ((read_size = fread(buffer, 1, sizeof (struct in_packet), fraw)) != 0) {
    if (read_size < 0) {
      perror("read");
      sleep(1);
    } else {
